...
def parse_modelark_error(response: httpx.Response) -> ModelArkAPIError:
    ...
    if status_code == 401 or "invalid" in error_code.lower() or "unauthorized" in error_code.lower():
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if status_code not in retriable_status_codes:
                        logger.error(f"{func.__name__} failed with non-retriable error: {modelark_error}")
                        raise modelark_error
                    ...
                    last_exception = e
                    logger.warning(f"{func.__name__} network error (attempt {attempt + 1}/{max_retries + 1}): {e}")
                
                # Don't sleep after last attempt
                if attempt < max_retries:
                    logger.info(f"Retrying {func.__name__} in {delay:.1f}s (attempt {attempt + 1}/{max_retries + 1})")
